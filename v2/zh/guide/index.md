...
参阅 [在线示例](https://riot.js.org/examples/plunker/?app=todo-app), 也可以浏览 [代码](https://github.com/riot/examples/tree/gh-pages/todo-app), 或下载[zip包](https://github.com/riot/examples/archive/gh-pages.zip).
...
参阅 [预处理器](/v2/zh/guide/compiler/#预处理器) 获取更多细节。
...
对同一个事件可以有多个事件监听器. 参阅 [可观察](/v2/zh/api/observable/) 获取关于事件的更多细节。
...
循环中的每一项将创建一个新的上下文([标签实例](/v2/zh/api/#标签实例))。如果有嵌套的循环，循环中的子标签都会继承父循环中定义了而自己未定义的属性和方法。
...
每一个循环项都是一个 [标签实例](/v2/zh/api/#标签实例). Riot 不会修改原始数据项，因此不会为其添加新的属性。
