...
### class指定でテンプレート変数を使うさい、オブジェクトで指定できる

<span class="tag red">&gt;=3.4.0</span>

タグのclass指定にオブジェクトを使うこともできます。例えば:
...
とすれば、`foo number animated baz zorro`と評価されます。Riotは与えられたオブジェクトのうち、値がtruthfulな（真として評価される）キー名のみを文字列として列記します。


### インラインstyle指定でテンプレート変数を使うさい、オブジェクトで指定できる

<span class="tag red">&gt;=3.4.0</span>

インラインの `style` 属性をオブジェクトの形で与えることができ、Riotはそれを自動的に（インラインstyle指定の）文字列に変換します。例えば:
