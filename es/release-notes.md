...
- Add the possibility to specify where riot will inject the custom tags [css in the DOM](/es/guide/#scoped-css)
...
- A new `<yield>` core tag allowing [html transclusion](/es/guide/#nested-html)
- A new [riot-tag](/es/guide/#html-elements-as-tags) attribute to use standard HTML elements as mount nodes
...
- [Server side rendering](/es/guide/#server-side-rendering)
...
- Support for [Scoped CSS](/es/guide/#scoped-css)
- Direct [access to nested tags](/es/api/#nested-tags) and their API via `tags` variable. For example: `tags.my_timer.clear()`
...
- New `unmount()` method and `children[]` property for [tag instances](/es/api/#tag-instance)
...
- [Compiler API](/es/api/compiler/) exposed to the public
- Global variables are supported in expressions, e.g. `{ location }`
- Customizable `.tag` extension, e.g. `riot --ext html`
- [Customizable brackets](/es/api/misc/#brackets), e.g. `riot.settings.brackets = '${ }'`
...
- Super fast [in-browser compilation](/es/guide/compile/) for: `<script type="riot/tag">`
- Built-in [Typescript support](/es/guide/compiler/#typescript)
- Ability to plug in a HTML pre-processor (along with JS processor)
- Built-in [Jade support](/es/guide/compiler/#jade)
- Ability to define [custom parsers](/es/api/#route-parser) for router.
